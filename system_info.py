        ...
        if key == 'memory':
            ...
            for mount, disk_data in value.items():
                ...
                for disk_key, disk_value in disk_data.items():
                    print(f"    {disk_key.replace('_', ' ').title()}: {disk_value} {'GB' if 'gb' in disk_key else '%' if disk_key == 'percentage' else ''}")
        elif key == 'network':
            print("\nNetwork Statistics:")
            for interface, net_data in value.items():
                print(f"\n  Interface: {interface}")
                for net_key, net_value in net_data.items():
                    print(f"    {net_key.replace('_', ' ').title()}: {net_value} {'MB' if 'mb' in net_key else ''}")
        else:
